/* This is vary basic program working with basic reader/writer timedlock
...
void *reader(void *args) {
  ...
  while (1) {
    ...
    while (pthread_rwlock_timedrdlock(&lock, &max_wait) != 0);
...
void *writer(void *args) {
  ...
  while (1) {
    struct timespec max_wait = {0, 0};
    max_wait.tv_sec += 1;
    while (pthread_rwlock_timedwrlock(&lock, &max_wait) != 0);
    if (strlen(reader_queue) == 0) {
